...
export default function JumelagePost() {
  ...
  const { id } = router.query; // ID de l'article actuel depuis l'URL
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setData(currentArticle); // Main article
        setRelatedArticles(filteredArticles); // Filtered related articles
      } catch (error) {
        setError(error);
      } finally {
        setLoading(false);
